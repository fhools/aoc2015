...
fn do_operation(grid:  &mut[[i32;1000]; 1000], op: Operation) {
    ...
    for x in start_x..(end_x+1) {
        for y in start_y..(end_y+1) {
            match op {
                ...
                Operation::Off(_,_) => {
                    grid[x as usize][y as usize] = max(grid[x as usize][y as usize] - 1, 0);
...
fn count_brightness(grid:  &[[i32;1000]; 1000]) -> i32 {
    // let mut cnt = 0;
    ...
    //     }
    // }
    grid.iter().flat_map(|s| s.to_vec()).fold(0, |total, b| total + b)
}

fn main() -> io::Result<()> {
    let lines = read_file("input.txt")?;
    let mut grid : [[i32; 1000]; 1000] = [[0i32; 1000]; 1000]; 
    //println!("lines: {:?}", lines);
    ...
    println!("Total brightness is {}", count_brightness(&grid));
    Ok(())
}
